...
function ForgotPassword() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setLoading(false);
    if (error) {
      setError(error.message.toLowerCase().includes("rate limit") ? "Too many email requests. Please wait a few minutes before trying again." : error.message);
    } else {
  ...
  return <div className="flex min-h-screen items-center justify-center bg-background p-6"><div className="w-full max-w-md rounded-3xl border bg-card p-8 shadow-lg">
    ...
    <form onSubmit={submit} className="mt-7 space-y-4"><input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" className="w-full rounded-xl border bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-primary" />
